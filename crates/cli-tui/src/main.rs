...
impl StatisticsCache {
    ...
    fn calculate(jobs: &[Job]) -> Self {
        ...
        let total_space_saved: u64 = jobs
            .iter()
        ...
        let compression_ratios: Vec<f64> = jobs
            .iter()
            .filter(|j| j.status == JobStatus::Success)
            .filter_map(|j| {
                if let (Some(orig), Some(new)) = (j.original_bytes, j.new_bytes) {
        ...
        let total_processing_time: i64 = jobs
            .iter()
        ...
        let estimated_pending_savings: u64 = jobs
            .iter()
        ...
        let mut recent_jobs: Vec<&Job> = jobs
            .iter()
            .filter(|j| j.status == JobStatus::Success || j.status == JobStatus::Failed)
            .collect();
        ...
        let recent_processing_times: Vec<i64> = recent_jobs
            .iter()
            .filter_map(|j| {
                if let (Some(started), Some(finished)) = (j.started_at, j.finished_at) {
                    Some((finished - started).num_seconds())
        ...
        let recent_compression_ratios: Vec<f64> = recent_jobs
            .iter()
            .filter(|j| j.status == JobStatus::Success)
            .filter_map(|j| {
                if let (Some(orig), Some(new)) = (j.original_bytes, j.new_bytes) {
        ...
        let recent_successful = recent_jobs
            .iter()
            .filter(|j| j.status == JobStatus::Success)
            .count();
...
impl LayoutConfig {
    ...
    fn from_terminal_size(size: Rect) -> Self {
        ...
        let table_columns = if is_too_small {
            ...
            vec![TableColumn::Status, TableColumn::File, TableColumn::Savings]
        } else if width >= 160 {
...
struct JobProgress {
    ...
    last_probe_duration: Option<f64>,
    encode_speed_x: Option<f64>, // input-seconds processed per wall-second
}
...
const PROBE_INTERVAL_SECS: i64 = 5;

/// Probe a media file (source or partial output) for its current duration in seconds.
/// Returns None if ffprobe is unavailable, fails, or the file doesn't exist yet.
...
/// Returns None if required metadata is not available
fn estimate_space_savings(job: &Job) -> Option<(f64, f64)> {
    // Returns (savings_gb, savings_percent)
    let orig_bytes = job.original_bytes?;
    if orig_bytes == 0 {
        return None;
    }
...
struct App {
    ...
    source_duration_cache: HashMap<String, f64>,
...
impl App {
    ...
    fn new(job_state_dir: PathBuf, temp_output_dir: PathBuf) -> Self {
        ...
        Self {
            ...
            source_duration_cache: HashMap::new(),
            job_state_dir,
            command_dir,
            temp_output_dir,
    ...
    fn filter_jobs<'a>(&'a self, jobs: &'a [Job]) -> Vec<&'a Job> {
        match self.ui_state.filter {
            ...
            JobFilter::Pending => jobs
                .iter()
                .filter(|j| j.status == JobStatus::Pending)
                .collect(),
            JobFilter::Running => jobs
                .iter()
                .filter(|j| j.status == JobStatus::Running)
                .collect(),
            JobFilter::Success => jobs
                .iter()
                .filter(|j| j.status == JobStatus::Success)
                .collect(),
            JobFilter::Failed => jobs
                .iter()
    ...
    /// Get and cache the source duration (seconds) for a job.
    fn get_source_duration_secs(&mut self, job: &Job) -> Option<f64> {
        if let Some(dur) = self.source_duration_cache.get(&job.id) {
            return Some(*dur);
        }

        let duration = probe_duration_seconds(&job.source_path)?;
        self.source_duration_cache.insert(job.id.clone(), duration);
        Some(duration)
    }
        ...
        match running_job {
            Some(job) => {
                ...
                if !self.command_dir.exists() {
                    std::fs::create_dir_all(&self.command_dir).with_context(|| {
                        format!(
                            "Failed to create command directory: {}",
                            self.command_dir.display()
                        )
                    })?;
                }
                ...
                let temp_file = self
                    .command_dir
                    .join(format!(".requeue-{}.json.tmp", job.id));
                ...
                std::fs::write(&temp_file, serde_json::to_string_pretty(&command)?).with_context(
                    || format!("Failed to write command file: {}", temp_file.display()),
                )?;

                std::fs::rename(&temp_file, &command_file).with_context(|| {
                    format!(
                        "Failed to rename command file: {} -> {}",
                        temp_file.display(),
    ...
    fn detect_job_progress(&mut self, job: &Job) {
        ...
        if let Ok(metadata) = fs::metadata(&temp_output) {
            ...
            let temp_file_modified_time = metadata
                .modified()
                .ok()
            ...
            let estimated_output_size =
                if current_temp_size > 1_000_000 && progress.bytes_per_second > 0.0 {
    ...
    fn refresh(&mut self) -> Result<()> {
        ...
        let now = Utc::now();
        let running_job_ids: Vec<String> = self
            .jobs
            .iter()
            .filter(|j| j.status == JobStatus::Running)
            .map(|j| j.id.clone())
        ...
        for job_id in &running_job_ids {
            if let Some(job) = self.jobs.iter().find(|j| j.id == *job_id).cloned() {
                ...
                if let Ok(metadata) = std::fs::metadata(&temp_output) {
                    let current_temp_size = metadata.len();
                    let mut progress =
                        self.job_progress.get(job_id).cloned().unwrap_or_else(|| {
                            JobProgress::new(temp_output.clone(), original_size)
                        });

                    // Calculate throughput
                    let time_delta_seconds =
                        (now - progress.last_updated).num_seconds().max(1) as f64;
                    if time_delta_seconds > 0.0 && current_temp_size > progress.temp_file_size {
                        let bytes_delta = current_temp_size - progress.temp_file_size;
                        progress.bytes_per_second = bytes_delta as f64 / time_delta_seconds;
                    }
                    ...
                    // Duration-driven estimation (preferred)
                    let mut estimated_output_size: Option<u64> = None;
                    if let Some(total_duration) = total_duration {
                        let should_probe = progress
                            .last_probe_time
                            .map(|t| (now - t).num_seconds() >= PROBE_INTERVAL_SECS)
                            .unwrap_or(true);

                        if should_probe {
                            if let Some(encoded_duration) = probe_duration_seconds(&temp_output) {
                                if encoded_duration > 0.1 {
                                    if let (Some(prev_dur), Some(prev_ts)) =
                                        (progress.last_probe_duration, progress.last_probe_time)
                                    {
                                        let wall_delta =
                                            (now - prev_ts).num_seconds().max(1) as f64;
                                        let dur_delta = (encoded_duration - prev_dur).max(0.0);
                                        if dur_delta > 0.0 {
                                            progress.encode_speed_x = Some(dur_delta / wall_delta);
                                        }
                                    }

                                    progress.last_probe_duration = Some(encoded_duration);
                                    progress.last_probe_time = Some(now);

                                    progress.progress_percent = (encoded_duration / total_duration
                                        * 100.0)
                                        .clamp(0.0, 100.0);

                                    if encoded_duration > 0.0 {
                                        let avg_output_bitrate = (progress.temp_file_size as f64
                                            * 8.0)
                                            / encoded_duration;
                                        estimated_output_size = Some(
                                            ((avg_output_bitrate * total_duration) / 8.0) as u64,
                                        );
                                    }
                    ...
                    // Fallback estimation when we don't have duration yet
                    let estimated_output_size = estimated_output_size
                        .or_else(|| {
                            if original_size > 0 {
                                calculate_estimated_output_size(&job)
                                    .or_else(|| Some((original_size as f64 * 0.5) as u64))
                            } else {
                                None
                            }
                        })
                    ...
                    progress.estimated_final_size =
                        Some(estimated_output_size.max(current_temp_size));

                    // Calculate current compression ratio (Task 13.2)
                    if current_temp_size > 0 && original_size > 0 {
                        progress.current_compression_ratio =
                            Some((original_size - current_temp_size) as f64 / original_size as f64);
                    }
                    ...
                    if let Some(frame_rate_str) = &job.video_frame_rate {
                        if let Some(fps) = parse_frame_rate(frame_rate_str) {
                            let duration_secs = total_duration.or_else(|| {
                                if let Some(bitrate) = job.video_bitrate {
                                    if bitrate > 0 && original_size > 0 {
                                        Some((original_size as f64 * 8.0) / bitrate as f64)
                                    } else {
                                        None
                                    }
                                } else {
                            ...
                            if let Some(duration_secs) = duration_secs {
                                progress.total_frames =
                                    JobProgress::calculate_total_frames(duration_secs, fps);
                                progress.frames_processed = progress.estimate_frames_processed();

                                if time_delta_seconds > 0.0 {
                                    let progress_delta =
                                        progress.progress_percent - progress.last_progress_percent;
                                    progress.current_fps = progress
                                        .calculate_current_fps(time_delta_seconds, progress_delta);
                                }
                            }
                        }
                    }
        ...
        self.job_progress
            .retain(|id, _| running_ids_set.contains(id));
        ...
        self.estimated_savings_cache
            .retain(|id, _| job_ids_set.contains(id));
...
fn main() -> Result<()> {
    ...
    let cfg = TranscodeConfig::load_config(config_path).context("Failed to load configuration")?;
...
fn render_simplified_view(f: &mut Frame, app: &App, area: Rect, _layout_config: &LayoutConfig) {
    ...
    let mut job_lines = vec![Line::from(Span::styled(
        format!("Total Jobs: {}", app.jobs.len()),
        Style::default()
            .fg(app.color_scheme.text_primary)
            .add_modifier(Modifier::BOLD),
    ))];
...
fn ui(f: &mut Frame, app: &mut App) {
    ...
    if layout_config.show_current_job
        && has_running_job
        && !layout_config.is_too_small
    ...
    if let Some(job) = app.jobs.iter().find(|j| j.status == JobStatus::Running) {
        ...
        let current_comp_ratio_str = if let Some(prog) = progress {
            ...
            if let Some(est_final) = prog.estimated_final_size {
                if prog.original_size > 0 {
                    let reduction =
                        (prog.original_size - est_final) as f64 / prog.original_size as f64;
        ...
        let info_lines = vec![
            format!(
                "  STAGE: {}  │  FILE: {}",
                stage_str,
                truncate_string(&file_name, 50)
            ),
            format!(
                "  VIDEO: {}  │  {}  │  {}  │  {}  │  {}",
                resolution, codec, bitrate, hdr_status, bit_depth
            ),
            format!(
                "  ORIG: {}  │  CURRENT: {}  │  EST FINAL: {}  │  RATIO: {}  │  QUALITY: {}",
                orig_size, new_size, est_final_size_str, current_comp_ratio_str, quality_str
            ),
            format!(
                "  SPEED: {}  │  FPS: {}  │  PROGRESS: {:.1}%  │  ETA: {}  │  ELAPSED: {}",
...
fn render_detail_view(f: &mut Frame, app: &App, area: Rect) {
    ...
    lines.push(format!(
        "   Web-like Content: {}",
        if job.is_web_like { "Yes" } else { "No" }
    ));
    ...
    if let Some(orig_bytes) = job.original_bytes {
        lines.push(format!(
            "   Original Size: {} ({} bytes)",
            format_size(orig_bytes, DECIMAL),
            orig_bytes
        ));
    } else {
    ...
    if let Some(new_bytes) = job.new_bytes {
        ...
        if let Some(orig_bytes) = job.original_bytes {
            if orig_bytes > 0 {
                ...
                lines.push(format!(
                    "   Space Saved: {} ({:.1}%)",
                    format_size(savings_bytes, DECIMAL),
                    savings_pct
                ));
...
fn render_job_table(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let header_cells: Vec<String> = layout_config
        .table_columns
        .iter()
    ...
    let rows: Vec<Row> = if filtered_jobs.is_empty() {
        ...
        let empty_cells: Vec<String> = layout_config
            .table_columns
            .iter()
            .enumerate()
            .map(|(idx, col)| {
        ...
        filtered_jobs
            ...
            .map(|(row_idx, job)| {
                ...
                let compression_ratio =
                    if let (Some(orig), Some(new)) = (job.original_bytes, job.new_bytes) {
                        if orig > 0 {
                ...
                let savings = if let (Some(orig), Some(new)) = (job.original_bytes, job.new_bytes) {
                    ...
                    let savings_str = if let Some(cached_estimate) =
                        app.estimated_savings_cache.get(&job.id)
                    {
                ...
                let cells: Vec<String> = layout_config
                    .table_columns
                    .iter()
                    .map(|col| match col {
    ...
    let widths: Vec<Constraint> = layout_config
        .table_columns
        .iter()
        .map(|col| col.width())
        .collect();
...
fn render_statistics_dashboard(f: &mut Frame, app: &App, area: Rect) {
    ...
    if show_sparklines {
        ...
        let savings_color = app
            .color_scheme
            .gradient_color_for_savings(stats.total_space_saved as f64 / 1_000_000_000.0);
        let compression_color = app
            .color_scheme
            .gradient_color_for_compression(stats.average_compression_ratio * 100.0);
        ...
        let pending_color = app
            .color_scheme
            .gradient_color_for_savings(pending_savings_gb);
...
fn render_trend_sparklines(f: &mut Frame, app: &App, area: Rect) {
    ...
    let proc_time_data: Vec<u64> = stats
        .recent_processing_times
        .iter()
    ...
    let proc_time_trend = if proc_time_data.len() >= 2 {
        let first_half_avg = proc_time_data
            .iter()
            .take(proc_time_data.len() / 2)
            .sum::<u64>() as f64
            / (proc_time_data.len() / 2).max(1) as f64;
        let second_half_avg = proc_time_data
            .iter()
            .skip(proc_time_data.len() / 2)
            .sum::<u64>() as f64
    ...
    let comp_ratio_data: Vec<u64> = stats
        .recent_compression_ratios
        .iter()
    ...
    let comp_ratio_trend = if comp_ratio_data.len() >= 2 {
        let first_half_avg = comp_ratio_data
            .iter()
            .take(comp_ratio_data.len() / 2)
            .sum::<u64>() as f64
            / (comp_ratio_data.len() / 2).max(1) as f64;
        let second_half_avg = comp_ratio_data
            .iter()
            .skip(comp_ratio_data.len() / 2)
            .sum::<u64>() as f64
...
fn render_status_bar(f: &mut Frame, app: &App, area: Rect) {
    ...
    let status_lines = vec![Line::from(line1), Line::from(line2)];
...
mod tests {
    ...
    fn test_total_space_saved_with_various_job_sets() {
        ...
        assert_eq!(
            ...
            "Empty job list should have 0 space saved"
        );
        ...
        let stats = StatisticsCache::calculate(&jobs);
        assert_eq!(
            stats.total_space_saved, 2_500_000_000,
            "Should sum all savings"
        );
        ...
        let stats = StatisticsCache::calculate(&jobs);
        assert_eq!(
            stats.total_space_saved, 500_000_000,
            "Only successful jobs should count"
        );
        ...
        assert_eq!(
            stats.total_space_saved, 0,
            "Jobs with missing metadata should not count"
        );
    }
    ...
    fn test_average_compression_ratio_calculation() {
        ...
        assert_eq!(
            ...
            "Empty job list should have 0 ratio"
        );
        ...
        let stats = StatisticsCache::calculate(&jobs);
        assert!(
            (stats.average_compression_ratio - 0.5).abs() < 0.001,
            "Average compression ratio should be 0.5, got {}",
        ...
        assert!(
            (stats.average_compression_ratio - expected_avg).abs() < 0.001,
        ...
        assert_eq!(
            stats.average_compression_ratio, 0.0,
            "Zero size should result in 0 ratio"
        );
    }
    ...
    fn test_success_rate_calculation() {
        ...
        assert_eq!(
            ...
            "Empty job list should have 0% success rate"
        );
        ...
        assert_eq!(
            ...
            "Only completed jobs should count (1/2 = 50%)"
        );
    }
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(
            ...
            "Running jobs without finished_at should not count"
        );
    }
    ...
    fn job_strategy() -> impl Strategy<Value = Job> {
        (
            ...
            .prop_map(|(id, has_metadata, status)| Job {
                id,
                source_path: PathBuf::from("/test/video.mkv"),
                output_path: None,
                status,
                ...
                video_frame_rate: if has_metadata {
                    Some("30/1".to_string())
                } else {
                    None
                },
                ...
                source_pix_fmt: if has_metadata {
    ...
    fn test_keyboard_shortcut_uniqueness() {
        ...
        let filter_shortcuts: Vec<&str> = shortcuts
            .iter()
        ...
        let navigation_shortcuts: Vec<&str> = shortcuts
            .iter()
        ...
        let detail_view_shortcuts: Vec<&str> = shortcuts
            .iter()
        ...
        let sort_shortcuts: Vec<&str> = shortcuts
            .iter()
        ...
        assert_eq!(sort_shortcuts[0], "s", "Sort shortcut should be 's'");
    }
    ...
    fn test_state_transitions() {
        ...
        assert_eq!(app.ui_state.view_mode, ViewMode::DetailView);
        assert_eq!(
            app.ui_state.detail_view_job_id,
            Some("pending1".to_string())
        );
        ...
        if let Some(idx) = app.ui_state.selected_index {
            assert!(
                idx < filtered_len,
                "Selection should be within bounds after adjustment"
            );
        }
    }
    ...
    fn test_refresh_cycle() {
        ...
        let progress = JobProgress::new(PathBuf::from("/tmp/test.tmp.av1.mkv"), 1_000_000_000);
        ...
        app.estimated_savings_cache
            .insert(pending_job.id.clone(), estimate);
