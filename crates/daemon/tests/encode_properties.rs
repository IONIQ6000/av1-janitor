...
use av1d_daemon::encode::common::{stream_mapping_flags, websafe_input_flags, pad_filter};
use av1d_daemon::encode::svt::build_svt_command;
use av1d_daemon::encode::aom::{build_aom_command, select_tiles, select_cpu_used};
    ...
    for max_concurrent in 1..=5 {
        ...
        for job_id in 0..num_jobs {
            ...
            let handle = tokio::spawn(async move {
                executor_clone.execute_job(|| async move {
        ...
        for handle in handles {
            handle.await.expect("Job task panicked").expect("Job failed");
        }
